...
/**
 * EigoMaster 全システム統合テスト実行スクリプト
 * 実行: node run-tests.js
 */
    ...
    this.tests.push({
      ...
      execute: async () => {
        ...
        try {
          ...
          const passed =
            version.startsWith('v16') || version.startsWith('v18') || version.startsWith('v20');
          return {
            passed,
  ...
  registerPhase4Tests() {
    ...
    this.tests.push({
      ...
      execute: async () => {
        ...
        try {
          ...
          const passed = easJson.build && easJson.build.ios;
          return {
            passed,
            duration: Date.now() - start,
            details: { iOSBuildConfigured: passed },
          };
        } catch (error) {
          return { passed: false, duration: Date.now() - start, error: 'eas.json解析エラー' };
        }
      },
    });
    ...
    this.tests.push({
      ...
      execute: async () => {
        ...
        try {
          const easJson = JSON.parse(fs.readFileSync('/Users/80dr/eigomaster/eas.json', 'utf8'));
          const passed = easJson.build && easJson.build.android;
          return {
            passed,
            duration: Date.now() - start,
            details: { androidBuildConfigured: passed },
          };
        } catch (error) {
          return { passed: false, duration: Date.now() - start, error: 'eas.json解析エラー' };
        }
    ...
    this.tests.push({
      ...
      execute: async () => {
        ...
        try {
          ...
          return { passed: false, duration: Date.now() - start, error: 'レスポンシブ設計なし' };
        }
      },
    });
  }
    ...
    this.tests.push({
      ...
      execute: async () => {
        ...
        try {
          ...
          const sizeGB = parseFloat(sizeOutput);

          // 1GB以下が目安
          const passed = sizeGB <= 1;

          return {
            passed,
            duration: Date.now() - start,
            details: { nodeModulesSize: sizeOutput },
          };
        } catch (error) {
          return { passed: false, duration: Date.now() - start, error: 'サイズ計測失敗' };
        }
      },
    });
      ...
      execute: async () => {
        ...
        try {
          ...
          return {
            ...
            duration: Date.now() - start,
            details: { storesWithCleanup: cleanupCount },
          };
        } catch (error) {
          return { passed: false, duration: Date.now() - start, error: String(error) };
        }
    ...
    this.tests.push({
      ...
      execute: async () => {
        ...
        try {
          ...
          return { passed: false, duration: Date.now() - start, error: 'RLS ポリシー未実装' };
        }
      },
    });
    ...
    this.tests.push({
      ...
      execute: async () => {
        ...
        try {
          ...
          return { passed: false, duration: Date.now() - start, error: 'XSS/CSRF 対策なし' };
        }
      },
    });
